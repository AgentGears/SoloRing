...
## Frozen Specification + Execution Results R2

**Status:** R2 freeze candidate (successor to frozen R1; specification only; execution not authorized by this document)
**Purpose:** certify the integrated M11–M16 production-world chain on the exact R2 correction baseline derived from published M16 before G8 Performance / Dialogue Co-Design
...
> **Does the R2 correction baseline (published M16 plus exactly the approved recovery correction), exercised as one accumulating film-production history, preserve the implemented M11–M16 contracts across reusable production, occurrence identity, world state, spatial binding, execution, Production Revision evolution, intra-Shot persistent consequences, recovery, and historical reproduction?**
...
# 0.1 R1 lineage (immutable)

R2 is the identity-corrected successor to frozen R1. R1 stays frozen
forever and records the failure on the published M16 baseline:
...
# 0.2 R2 certifying baseline (the approved recovery correction)
...
Mechanical requirements before freeze (recorded in
correction/correction-head-identity.txt):
...
No result-driven amendment of R2 is permitted.
...
The canonical R2 file is the exact file in the freeze set at this
exact archive-root-relative path ("freeze-set root" means the root
of the presented freeze archive / the `FREEZESET.sha256` tree):

```text
spec/SoloRing-Post-M16-Integrated-Sequence-Regression-R2.md
```
...
R2 inherits the frozen R1's oracle hierarchy verbatim (R1 pinned exactly the M16 R7 normative foundation).
...
No v1.7 architecture artifact or Roadmap v1.5 is a normative R2 oracle.
...
older v1.1/local rerun material
        =
historical/non-authoritative context for R2
```

If an older untracked rerun conflicts with the certified v1.6 rerun, the old rerun does not control R2.
...
Before R2 freezes, the exact oracle bytes used by the regression must be collected into one immutable evidence input set:

```text
<freeze-set root>/                      (archive root)
  spec/
    SoloRing-Post-M16-Integrated-Sequence-Regression-R2.md
...
```text
B. place exact copies in the R2 freeze input set,
...
R2 carries THREE identities that must never be collapsed: the certified M16 implementation (the R1 closure object), the published M16 baseline (R1's failed certifying target), and the R2 correction baseline (§0.2, the new certifying target).
...
The R1 certifying regression executed against the published M16 identity and FAILED (§0.1). The R2 certifying regression executes against the **R2 correction baseline** of §0.2; the §4.3 proof below is retained as lineage proving how that baseline's parent was constructed.
...
R2 must prove both history legs and their tree-equivalence join.
...
Run from a clean detached worktree created from the exact R2 CERTIFYING_COMMIT (§0.2).
...
CERTIFYING_TREE=941d14cda84d0186431a80c0f9cbdc3fa978296b
PARENT_COMMIT=488031a2b7d0070d23425bbe04b86d9f9f16e0bd
# identity: the R2 certifying baseline (§0.2), bound by frozen
...
# fresh DB upgrades to exactly 0017
TMPDB="$(mktemp -d)/guard.db"
cd "$CK/server" && python -m alembic -x db_url="sqlite:///$TMPDB" upgrade head
...
R2 executes new integrated evidence on the accumulating Project.

### INHERITED_CERTIFIED

The exact R2 correction baseline already provides sufficient certifying local evidence and R2 pins that evidence instead of duplicating it.
...
R2 must exercise the published M15 flow:
...
Therefore R2 freezes one narrowly scoped exception to §7.1.
...
After insertion, the frozen Generation/Take evidence-input rows become immutable accumulated Project history: later R2 cells may read and cite them, including historical-leakage tests, but no later cell may mutate, replace, or re-seed them.
...
ZERO64 = 0000...0  (64 zeros)
PARAMS = {"schema_version":1}
SPECJ  = {"schema_version":1,"fixture":true}
SPECH  = sha256(canonical_json_bytes(SPECJ))            (derived)
...
PM16:HISTORY and PM16:NEG:C cells re-read these rows and re-derive
the same root to prove immutability across the whole sequence.
...
In addition to inherited local milestone evidence, R2 directly executes:
...
These remain REQUIRED R2 cells even if their constituent local invariants already have predecessor proof-map coverage; their oracle-case mapping lives in the OCL rows.

---

# 18. Inherited M16 proof-map relationship

R2 is not an extension of the frozen M16 164-cell universe.

The 164-cell map remains unchanged.

R2 may cite exact M16 cells as `INHERITED_CERTIFIED`, but creates its own namespace:
...
The complete R2 cell universe is frozen independently before execution.

No R2 cell is retrospectively added to `M16:*`.
...
```text
post-m16-integrated-r1-evidence/
  oracles/
...
The certifying Tier-A run occurs against the clean detached **R2 correction baseline** (§0.2) — the published M16 baseline plus exactly the approved recovery correction.

The R2 specification, harness, and evidence may not change that system-under-test tree before certification.
...
R2 specification + regression harness
    may be committed as an evidence/test-only change
```

Such a later commit is not the commit certified by R2.
...
Only a complete R2 PASS may state:

> **POST-M16 INTEGRATED PRODUCTION-WORLD REGRESSION: PASS.
> The implemented M11–M16 production-world chain remains coherent on the exact R2 correction baseline derived from published M16. The roadmap prerequisite for entering G8 is satisfied. G8 remains OPEN. No M17 implementation is authorized.**

R2 carries forward, but does not answer:
...
> **SoloRing's R2 correction baseline (the published M16 baseline plus exactly the two-file recovery correction of §0.2) preserves the implemented M11–M16 production-world program as one coherent accumulated system. Reusable Production Revisions and surviving occurrence identities remain stable through explicit compatibility-gated evolution; persistent state survives lawful realization revision changes; later evolved occurrences remain valid targets for new intra-Shot consequences; intra-Shot changes preserve true Shot-start state and persist only through explicit adoption into existing continuity authority; adopted consequences become ordinary downstream world state consumable by event-free M13/M14 capture and execution; current production evolution cannot reinterpret captured history; and recovery and historical reproduction operate from retained immutable authority rather than current/latest state. Active schema-7 event realization remains deliberately unsupported and fails closed until a later reviewed Performance execution milestone provides that capability. This claim is made about the correction baseline; its exact two-file departure from published M16 is pinned in §0.2.**
...
R2 is freezeable only when all are complete:

* [x] lineage/* and correction/* byte-verified against FREEZESET.sha256; embedded lineage/correction identities mechanically equal the §0.1/§0.2 pins;
* [x] correction ancestry + two-file delta + blob identities + patch SHA + commit-object bytes mechanically re-verified;
...
* [x] canonical freeze-set path for this exact R2 file recorded;
...
**Status:** executed — Tier-A Run 1 closed FAIL (2026-09-19).

Part I remains byte-identical to the frozen identity in §A. Only this Part II records the authorized Tier-A execution results and the formal closure.

## A. Frozen specification identity

```text
part_i_byte_count: 70102
part_i_sha256: 5ad51e1dbe5bbe68753d1112c14a4eb66e4f526325331bbb2df63eea61682dad
whole_preexecution_file_sha256:
  84535e34060ad38fc1a0d1e916ab26fbe730cf3aa6863fb04ae8be7194cdf1e3
...
```text
r2_parent_commit: 488031a2b7d0070d23425bbe04b86d9f9f16e0bd
correction_commit: 8199e46da6b8592f606725059601b402ab2874b0
correction_tree: 941d14cda84d0186431a80c0f9cbdc3fa978296b
correction_delta_proof: parent + 2 files + blobs + patch SHA
certified_implementation_commit: 9e703806304063110a07458c3d3e0b0124c6ab14
certified_implementation_tree: ca52da2629502ac813538854bd812f2886a99c7b

pre_M16_main: 30ea135f3b2339491e9b36eaee2d0d8bc4ab8585
branch_publication_commit_1: e235b655dff382af5d695c50b10622c3fea8a97f
branch_publication_tip: 44936bfc2a3ac565621198bd84ea0370cbb753d9

published_M16_commit: 488031a2b7d0070d23425bbe04b86d9f9f16e0bd
published_M16_tree: a2a3c3e480d20b10e472812505e4370a865a8d6d
M16_tag_object: 4a890260448021419168ea51da6321d5b1e3867c
M16_tag_peeled_commit: 488031a2b7d0070d23425bbe04b86d9f9f16e0bd
```

## C. Oracle verification

```text
six_artifact_verification: PASS (byte-equal §2.1 pins)
v1.0_provenance_verdict: VERIFIED_ORIGINAL
v1.0_sha256: d25116eb0d61eaac2012e15872bf6e22c45c0644466669af3c913ca65cfec996
v1.0_v1.6_reconciliation_verdict: v1.6 preserves the unchanged scenario
```

## D. Tier A

```text
pre_guard: PASS (guard_exit=0; frozen §6.4 wrapper verbatim, contemporaneous)
execution: 59 REQUIRED PASS / 5 REQUIRED NOT_EXECUTED / 3 OPTIONAL NOT_EXECUTED; fail-fast on fixture/harness incompatibility encountered while entering RECOVERY:01 — RECOVERY:01 itself remains NOT_EXECUTED and the thrown exception is recorded as HARNESS: FAIL in the evidence ledger
post_guard: PASS (guard_exit=0; HEAD/tree/checkout residue clean)
disposition: FAIL
```
...
```text
status: NOT_RUN (not authorized; Tier-B files omitted)
runtime_identity:
pre_guard:
execution:
post_guard:
...
```text
manifest_sha256: 3d55aa59f13e736751ca230e6aa387d5e3c7f37116d176df530ec0091bdfa460
manifest_bytes: 19178
entry_count: 103
```
...
run1_archive: SoloRing-Post-M16-R2-TierA-Run1-2026-09-19.zip
run1_archive_bytes: 396506
run1_archive_sha256: 829f0b0666e29a1750066c661235f4fef476e827cfde41197744bc7339a1ff89
failure_surface: frozen CORE-3 fixture incompatible with whole-history recovery
product_defect: no
```
...
```text
NOT EVALUATED — G8 remains closed; no R2 PASS exists
