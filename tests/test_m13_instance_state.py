...
async def test_m13_impact_02(client):
    ...
    base, cid, oid = await _world(client, tag=b"m13-imp02")
    await _feature(client, oid)
    r = await client.post(
    ...
    assert pv["live_blocking_references"] == [
        {"consumer": "production_instance_feature",
         "id": pv["live_blocking_references"][0]["id"],
         "occurrence_id": oid}]
    r = await client.post(
    ...
    engine = client._transport.app.state.engine
    async with engine.connect() as conn:
        fidrow = (await conn.execute(text(
            "SELECT id FROM production_instance_features "
            "WHERE occurrence_id = :o"), {"o": oid})).first()
    r = await client.delete(f"/production-instance-features/{fidrow.id}")
    assert r.status_code == 200, r.text
