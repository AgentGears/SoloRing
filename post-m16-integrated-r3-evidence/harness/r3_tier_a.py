"""R3 Tier-A certifying sequence driver (frozen §7.2).

One accumulating Project, one session-scoped engine/app/client, nine
ordered phases, fail-fast, per-cell evidence records. Run from OUTSIDE
the certification checkout with the checkout as PYTHONPATH/cwd:

    python r3_tier_a.py --ck <checkout> --ev <evidence-dir> \
        --freeze <freeze-set-dir> [--until N]

Part I is immutable; this harness implements it against the R3
correction baseline (§0.2 — the unchanged R2 product baseline).
"""
...
def build_full_ledger() -> list[dict]:
    ...
    for cid, tier, gate, expected in cell_rows:
        ...
        ledger.append({
            ...
            "evidence_file": (f"cells/{cid.replace(':', '_')}.json"
                              if rec else None),
        })
    return ledger
